<!--
  Copied from desktop/frontend/src/ui/status-badge/SPEC.md by
...
A run's state, as a tinted pill carrying the word for that state. Six status
...
- `span.sd-badge[data-status]` — the pill. `padding-block: 1px`,
  ...
  the text is the hue at full strength.
...
| State | What changes |
|---|---|
| rest | The hue named by `data-status`, or the lane's hue when the badge is inside a `.sd-lane`. |
...
- `--sd-st-queue` / `--sd-st-live` / `--sd-st-blocked` / `--sd-st-triaged` / `--sd-st-done` / `--sd-st-failed` — the six hues
...
- **Do** ship the word with the hue, always. `STATUS_WORDS` is part of the
...
- **Do** say what the state means to a reader: "Needs input", not "Blocked".
