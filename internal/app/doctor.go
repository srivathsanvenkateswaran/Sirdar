...
// configuration, the platform's opener and credential store, the PATH the
// process is looking for binaries on, the agent binary, every configured
// source, which MCP servers a session will see, the notes directory and
// the note templates.
...
func RunDoctor(ctx context.Context, cfg *config.Config) []Check {
	...
	// Before the provider rows: when a provider binary cannot be found,
	// this is the row that says where Sirdar looked, and the ones below
	// are the symptom.
	checks = append(checks, environmentCheck(cfg))
