...
//
// The test is provider.ResolveWithin, the same confinement the permission
// policy applies to a write it is asked about — a relative path is joined
// onto the root, symlinks resolve before the comparison, and a leading "~"
// is refused rather than read as a directory of that name. A write this
// client was never asked about is judged by the rule it would have been
// judged by.
func (s *session) writesWithinRoot(call trackedCall) bool {
	if len(call.paths) == 0 {
		return false
	}
	if strings.TrimSpace(s.spec.Cwd) == "" {
		return false
	}
	for _, path := range call.paths {
		if _, err := provider.ResolveWithin(s.spec.Cwd, path); err != nil {
			return false
		}
	}
	return true
