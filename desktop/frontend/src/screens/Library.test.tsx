...
/** The thirty-one components, in build order, as the sections name them. */
const SECTIONS = [
  'Button',
  'Pill nav',
  'Segmented control',
...
function paint(theme: 'light' | 'dark', dir: 'ltr' | 'rtl'): void {
  if (theme === 'dark') fireEvent.click(screen.getByRole('radio', { name: 'Dark' }))
  if (dir === 'rtl') fireEvent.click(screen.getByRole('radio', { name: 'RTL' }))
}
...
describe('the asset library', () => {
  describe.each(FRAMES)('painted %s', (_, theme, dir) => {
    it('renders every one of the thirty-one sections, each with a specimen', () => {
      render(<Library />)
      paint(theme, dir)
      ...
      for (const section of sections) {
        ...
        expect(section.querySelector('.lib-section__body')?.children.length, name).toBeGreaterThan(
          0,
        )
      }
    })
  ...
  it('heads the page with its name, a lede and the two switches, outside the frame', () => {
    ...
    const bar = container.querySelector<HTMLElement>('.lib__bar')
    ...
    expect(within(bar).getAllByRole('radiogroup').map((g) => g.getAttribute('aria-label'))).toEqual([
      'Theme',
      'Direction',
    ])
  ...
  it('shows the run card in every state its spec lists', () => {
    ...
    expect(within(frame()).getByRole('button', { name: 'OMNI-2513: OMNI-2513' })).toBeInTheDocument()
  })
  ...
  it('shows the item row opening and keeping its own control, in every tone', () => {
    ...
    expect(within(done as HTMLElement).getByRole('button', { name: 'Open note' })).toBeInTheDocument()
  })
  ...
  it('shows the smaller pieces in the states their specs list', () => {
    ...
    expect(within_.getAllByRole('searchbox').some((box) => (box as HTMLInputElement).disabled)).toBe(
      true,
    )
    // Toggle: on, off, and disabled in both positions.
    const switches = within_.getAllByRole('switch')
    expect(switches.filter((s) => s.getAttribute('aria-checked') === 'true').length).toBeGreaterThan(
      0,
    )
    expect(
    ...
    expect(within(within_.getByRole('radiogroup', { name: 'Kind' })).getAllByRole('radio')).toHaveLength(
      4,
    )
  })
