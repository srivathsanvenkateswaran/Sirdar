...
describe('the layout dispatch', () => {
  ...
  it('draws the Document layout for Workbench until that layout lands, and the header switcher moves between them', async () => {
    setSessionLayout('workbench')
    mount()
    await screen.findByRole('heading', { name: 'SBX-1' })
    expect(screen.getByTestId('session-document')).toBeInTheDocument()
    expect(screen.getByRole('radio', { name: 'Workbench' })).toBeChecked()
  })
})
